...
export async function printToKitchen() {
    ...
    if (printers.length === 0) {
        toast('الرجاء تعيين طابعات لزر المطبخ في الإعدادات', 'te');
        return;
    }
...
export async function settleInvoice() {
    ...
    if (!Object.keys(oB).length && !Object.keys(oC).length) { 
    ...
    if (printers.length === 0) {
        toast('الرجاء تعيين طابعات لزر تسديد الحساب في الإعدادات', 'te');
        return;
    }
    ...
    for (let id in combined) { 
        let s = combined[id].price * combined[id].quantity; 
        total += s; 
    ...
    Promise.all([
        DB.removeData('tablesB/' + k), 
        DB.removeData('tablesC/' + k), 
        DB.removeData('msgA/' + k), 
        DB.removeData('msgB/' + k), 
        DB.removeData('read_counters/' + k)
...
export function updateSales() { 
    ...
    directOrders.forEach(order => {
        if (order.settled_by === state.myUser && sameDay(order.settled_at)) {
            t += order.total || 0;
        }
    });
...
export function printDirectReceipt(items, total) {
    const printers = window.App.getButtonPrinters('direct');
    if (printers.length === 0) {
        toast('الرجاء تعيين طابعات لزر التسوق المباشر في الإعدادات', 'te');
        return false;
    }
    return printToSelectedPrinters(printers, { 
