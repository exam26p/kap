...
function getMp3(f) {
    ...
    if (!sndCache[f]) { 
        sndCache[f] = new Audio(f); 
        sndCache[f].volume = 0.6; 
        sndCache[f].onerror = function () { this.src = sndBase + f; }; 
    }
    return sndCache[f];
}
...
function playClickProg(idx) { 
    try { 
        const c = clickFreqs[idx % clickFreqs.length]; 
        ...
        o.start(); 
        o.stop(ac.currentTime + c.d + 0.01); 
    } catch (e) { } 
}
...
export function playClickSnd() { 
    ...
    if (cs === 'off') return; 
    if (cs.startsWith('__click')) { 
        const n = parseInt(cs.replace('__click', '')) - 1; 
        playClickProg(isNaN(n) ? 0 : n); 
    } else playSnd(cs); 
}
...
export function alarmOrder(on) { 
    if (on) { 
        if (!state.orderAlarm) state.orderAlarm = setInterval(() => playOnce('order'), 3000); 
    } else { 
        clearInterval(state.orderAlarm); 
        state.orderAlarm = null; 
    } 
}

export function alarmMsg(on) { 
    if (on) { 
        if (!state.msgAlarm) state.msgAlarm = setInterval(() => playOnce('msg'), 4000); 
    } else { 
        clearInterval(state.msgAlarm); 
        state.msgAlarm = null; 
    } 
}
...
export function drawDash(cx, x1, x2, y, w) { 
    cx.strokeStyle = '#333'; 
    cx.lineWidth = w || 1; 
    cx.setLineDash([4, 3]); 
    cx.beginPath(); 
