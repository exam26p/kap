...
export function printQR() { playOnce('print'); document.body.classList.add('qr-print'); window.print(); setTimeout(() => document.body.classList.remove('qr-print'), 500); } 
...
export function loadSettingsUI() { if (document.getElementById('sName')) document.getElementById('sName').value = state.settings.restaurantName || ''; if (document.getElementById('sInvoiceUrl')) document.getElementById('sInvoiceUrl').value = state.settings.invoiceQrUrl || ''; if (document.getElementById('sOrderSnd')) document.getElementById('sOrderSnd').value = state.settings.orderSound || 'off'; if (document.getElementById('sMsgSnd')) document.getElementById('sMsgSnd').value = state.settings.msgSound || 'off'; if (document.getElementById('sPrintSnd')) document.getElementById('sPrintSnd').value = state.settings.printSound || 'off'; if (document.getElementById('sClickSnd')) document.getElementById('sClickSnd').value = state.settings.clickSound || 'off'; syncLogoDisplay(state.settings.restaurantLogo || ''); state.pendingLogoData = null; } 
...
export function searchStats() { const user = document.getElementById('stUser').value, from = document.getElementById('stFrom').value, to = document.getElementById('stTo').value; if (!from && !to && !user) { toast('حدد فلتر واحد على الأقل', 'te'); return; } let invMap = {}, itemsSum = {}, grand = 0; for (let id in state.dD) { const o = state.dD[id]; if (!o) continue; if (user && o.settled_by !== user) continue; if (!inRange(o.settled_at, from, to)) continue; const price = parseFloat(o.price) || 0, qty = parseInt(o.quantity) || 1, sub = price * qty; const ik = o.table_number + '_' + o.settled_at; if (!invMap[ik]) invMap[ik] = { tableNumber: o.table_number, settledAt: o.settled_at, totalPrice: 0 }; invMap[ik].totalPrice += sub; if (!itemsSum[o.name]) itemsSum[o.name] = { quantity: 0, total: 0 }; itemsSum[o.name].quantity += qty; itemsSum[o.name].total += sub; grand += sub; } const directOrders = JSON.parse(localStorage.getItem('direct_orders_archive') || '[]'); directOrders.forEach(order => { if (user && order.settled_by !== user) return; if (!inRange(order.settled_at, from, to)) return; grand += order.total || 0; const ik = 'direct_' + order.settled_at; if (!invMap[ik]) invMap[ik] = { tableNumber: 'سفري', settledAt: order.settled_at, totalPrice: 0 }; invMap[ik].totalPrice += order.total || 0; if (!itemsSum['تسوق مباشر']) itemsSum['تسوق مباشر'] = { quantity: 0, total: 0 }; itemsSum['تسوق مباشر'].quantity += order.items || 1; itemsSum['تسوق مباشر'].total += order.total || 0; }); const invList = Object.values(invMap).sort((a, b) => new Date(a.settledAt) - new Date(b.settledAt)); state.lastStats = { targetUser: user || 'الكل', dateFrom: from || 'البداية', dateTo: to || 'اليوم', invoices: invList, summary: itemsSum, totalSum: grand, ordersCount: invList.length }; const c = document.getElementById('statsList'); if (!invList.length) { c.innerHTML = '<p style="color:var(--text-muted);text-align:center;padding:36px;">لا توجد مبيعات.</p>'; state.lastStats = null; return; } let h = '<h4 style="color:var(--accent);margin-bottom:12px;font-size:13px;"><i class="fas fa-receipt"></i> الفواتير (' + invList.length + '):</h4>' + '<div style="max-height:160px;overflow-y:auto;background:var(--bg-input);border-radius:10px;padding:10px;margin-bottom:18px;border:1px solid var(--border);">'; invList.forEach(inv => { const t = new Date(inv.settledAt).toLocaleTimeString('ar-IQ', { hour: '2-digit', minute: '2-digit' }), d = new Date(inv.settledAt).toLocaleDateString('ar-IQ'); h += '<div class="s-inv-row">' + '<span><b>' + (inv.tableNumber === 'سفري' ? 'تسوق مباشر' : 'طاولة ' + inv.tableNumber) + '</b> <small style="color:var(--text-muted);margin-right:6px;">(' + d + ' ' + t + ')</small></span>' + '<span style="color:var(--green);font-weight:700;">' + fmt(inv.totalPrice) + ' د.ع</span>' + '</div>'; }); h += '</div><h4 style="color:var(--teal);margin-bottom:12px;font-size:13px;"><i class="fas fa-box-open"></i> ملخص المواد:</h4>'; Object.entries(itemsSum).forEach(([name, data]) => { h += '<div class="s-item-card">' + '<div>' + '<div class="s-item-name"><i class="fas fa-utensils" style="color:var(--teal);margin-left:5px;font-size:12px;"></i>' + name + '</div>' + '<div class="s-item-count">العدد: ' + data.quantity + '</div>' + '</div>' + '<span class="s-item-total">' + fmt(data.total) + ' د.ع</span>' + '</div>'; }); h += '<div class="s-grand">' + '<div class="s-label">المجموع النهائي</div>' + '<div class="s-value">' + fmt(grand) + ' دينار عراقي</div>' + '</div>'; c.innerHTML = h; } 
...
export function initCaptainChat() {  
    ...
    DB.listenCaptainChat(state.myUser, data => {  
        ...
        // فحص إذا كانت هناك رسالة جديدة من الكابتن
        const latestMsg = m[m.length - 1];
        if (latestMsg.sender === 'captain' && latestMsg._id !== lastCaptainMsgKey) {
            if (lastCaptainMsgKey !== null) { // التأكد من عدم التنبيه عند تحميل الصفحة أول مرة
                playOnce('msg'); // تشغيل صوت الرسائل
                toast('رسالة جديدة من الكابتن: ' + latestMsg.text.substring(0, 30), 'ti'); // إشعار مرئي
                updateCaptainBadge(true); // إظهار نقطة حمراء على الزر
            }
            lastCaptainMsgKey = latestMsg._id; // تحديث آخر رسالة
        }
