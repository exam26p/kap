...
import { fmt, playClickSnd, playOnce, toast, doBrowserPrint, drawDash, inRange } from './utils.js';
...
export async function generateQRCodes() {
    ...
    const from = parseInt(document.getElementById('qrFrom').value) || 1, 
          to = parseInt(document.getElementById('qrTo').value) || from; 
    const total = Math.max(0, to - from + 1), 
          showV = document.getElementById('qrShowVer').checked, 
          showH = document.getElementById('qrShowHint').checked;
    ...
    btn.disabled = true; 
    const pw = document.getElementById('qrProgress'); 
    pw.classList.add('show'); 
    const grid = document.getElementById('qrsGrid'); 
    grid.innerHTML = ''; 
    state.qrData = []; 
    ...
    for (let i = 0; i < total; i++) { 
        const tn = from + i, 
              url = base + '?table=' + tn + (ver ? '&version=' + ver : ''); 
        try { 
            await DB.setData('tablesA/table_' + tn, { 
                table_number: tn, 
                qr_link: url, 
                version_number: ver || null, 
                status: 'active', 
                created_at: new Date().toISOString() 
            }); 
        } catch (e) { } 
        state.qrData.push({ tn, url, ver, showV, showH }); 
    ...
    document.getElementById('qrTools').style.display = 'flex'; 
    btn.disabled = false; 
...
function makeQR(tn, url, ver, showV, showH) { 
    ...
    new QRCode(document.getElementById('qr_' + tn), { 
        text: url, 
        width: 140, 
        height: 140, 
        colorDark: '#000000', 
...
export function refreshQRCards() { 
    const showV = document.getElementById('qrShowVer').checked, 
          showH = document.getElementById('qrShowHint').checked, 
          cols = parseInt(document.getElementById('qrCols').value) || 4; 
    document.getElementById('qrsGrid').className = 'qrs-grid c' + Math.min(cols, 4); 
    document.getElementById('qrsGrid').innerHTML = ''; 
    state.qrData.forEach(d => makeQR(d.tn, d.url, d.ver, showV, showH)); 
...
export function printQR() { 
    ...
    window.print(); 
    setTimeout(() => document.body.classList.remove('qr-print'), 500); 
}

// ===== التسوق المباشر =====
export function renderDirectGrid() {
    const search = document.getElementById('directSearch').value.trim().toLowerCase(), 
          catFilter = document.getElementById('directCatFilter').value, 
          grid = document.getElementById('directGrid');
    state.directAllItems = []; 
    const filterSel = document.getElementById('directCatFilter'); 
    const cv = filterSel.value; 
    filterSel.innerHTML = '<option value="">جميع الأقسام</option>';
    for (let catId in state.directMenuData) { 
        filterSel.innerHTML += '<option value="' + catId + '">' + state.directMenuData[catId].name + '</option>'; 
        if (state.directMenuData[catId].items) { 
            for (let iid in state.directMenuData[catId].items) { 
                const it = state.directMenuData[catId].items[iid]; 
                state.directAllItems.push({ ...it, catId, catName: state.directMenuData[catId].name, _id: iid }); 
            } 
        } 
    } 
    filterSel.value = cv;
    let filtered = state.directAllItems; 
    if (search) filtered = filtered.filter(i => i.name.toLowerCase().includes(search)); 
    if (catFilter) filtered = filtered.filter(i => i.catId === catFilter);
    grid.innerHTML = ''; 
    if (!filtered.length) { 
    ...
    filtered.forEach(it => { 
        const imgSrc = it.image && it.image.length > 20 ? it.image : ''; 
        const key = it.catId + '_' + it._id; 
        const qty = state.directCart[key] ? state.directCart[key].quantity : 0; 
...
function updateDirectBar() { 
    ...
    let count = 0, total = 0; 
    for (let k in state.directCart) { 
        const parts = k.split('_'); 
        const catId = parts[0]; 
        let item = null; 
        if (state.directMenuData[catId] && state.directMenuData[catId].items) { 
            const itemId = parts.slice(1).join('_'); 
            if (state.directMenuData[catId].items[itemId]) item = state.directMenuData[catId].items[itemId]; 
        } 
        if (item) { 
    ...
    document.getElementById('directCount').textContent = count + ' صنف'; 
    document.getElementById('directTotal').textContent = fmt(total) + ' دينار'; 
}
...
export function payDirectOrder() { 
    const keys = Object.keys(state.directCart); 
    ...
    keys.forEach(k => { 
        ...
        const catId = parts[0]; 
        const itemId = parts.slice(1).join('_'); 
        const cat = state.directMenuData[catId]; 
        if (cat && cat.items && cat.items[itemId]) { 
            const it = cat.items[itemId]; 
            const qty = state.directCart[k].quantity; 
    ...
    // استخدام دالة الطباعة الموحدة من panelOps
    ...
    const ops = []; 
    items.forEach(it => { 
        ops.push(DB.pushData('tablesD/archive_orders', { 
            table_number: 0, 
            name: it.name, 
            price: it.price, 
            quantity: it.qty, 
    ...
    const directOrdersArchive = JSON.parse(localStorage.getItem('direct_orders_archive') || '[]');
    directOrdersArchive.push({ settled_by: state.myUser, settled_at: sa, total, items: items.length });
    ...
    }).catch(() => toast('خطأ في الحفظ', 'te')); 
}

// ===== الإعدادات =====
export function syncPrinterSelects() { 
    const selectIds = ['rKitchen', 'rCashier', 'stPrinter', 'directPrinter'];
    ...
    if (document.getElementById('rKitchen')) document.getElementById('rKitchen').value = state.routing.kitchenPrinterIp; 
    if (document.getElementById('rCashier')) document.getElementById('rCashier').value = state.routing.cashierPrinterIp; 
}
    ...
    for (let id in state.dPrinters) {
        c.innerHTML += '<div class="printer-row">' +
            '<div style="color:var(--text-secondary);font-size:13px;">' +
                ...
                '<b>' + state.dPrinters[id].name + '</b> ' +
                '<span style="color:var(--text-muted);">(' + state.dPrinters[id].ip + ')</span>' +
            '</div>' +
            '<div style="display:flex;gap:8px;">' +
                '<button class="btn-secondary btn-sm" onclick="App.pingP(\'' + state.dPrinters[id].ip + '\')"><i class="fas fa-wifi"></i> فحص</button>' +
...
export function pingP(ip) { 
    fetch('http://localhost:5000/api/ping-printer', { 
        method: 'POST', 
        headers: { 'Content-Type': 'application/json' }, 
        body: JSON.stringify({ ip }) 
    }).then(r => r.json()).then(d => toast('[' + ip + ']: ' + d.message, d.message.includes('متصل') ? 'ts' : 'te')).catch(() => toast('تعذر الاتصال بسيرفر الطباعة', 'te')); 
}

export function addPrinter() { 
    const n = document.getElementById('pName').value.trim(), 
          ip = document.getElementById('pIP').value.trim(); 
    if (!n || !ip) { toast('أدخل اسم الطابعة والـ IP', 'te'); return; } 
    DB.pushData('printers_config', { name: n, ip }).then(() => { 
...
export function delP(id) { 
    if (confirm('حذف هذه الطابعة؟')) DB.removeData('printers_config/' + id); 
}
...
export function syncLogoDisplay(dataUrl) { 
    const img = document.getElementById('logoPreview'), 
          ph = document.getElementById('logoPlaceholder'), 
          hImg = document.getElementById('headerLogo'), 
          hPh = document.getElementById('headerLogoPlaceholder'), 
          rmBtn = document.getElementById('removeLogoBtn'); 
...
export function saveSettings() { 
    const n = document.getElementById('sName').value.trim(), 
          invUrl = document.getElementById('sInvoiceUrl').value.trim(), 
          os = document.getElementById('sOrderSnd').value, 
          ms = document.getElementById('sMsgSnd').value, 
          ps = document.getElementById('sPrintSnd').value, 
          cs = document.getElementById('sClickSnd').value, 
          ki = document.getElementById('rKitchen') ? document.getElementById('rKitchen').value : '', 
          ci = document.getElementById('rCashier') ? document.getElementById('rCashier').value : ''; 
    const ns = { 
    ...
    Promise.all([DB.updateData('app_settings', ns), DB.setData('buttons_routing', { kitchenPrinterIp: ki, cashierPrinterIp: ci })]).then(() => { 
        state.settings = { ...state.settings, ...ns }; 
        state.routing = { kitchenPrinterIp: ki, cashierPrinterIp: ci }; 
        document.getElementById('brandName').textContent = state.settings.restaurantName; 
...
export function syncStatsUsers() { 
    const s = document.getElementById('stUser'); 
    if (!s) return; 
    const cv = s.value; 
    s.innerHTML = '<option value="">الكل</option>'; 
    for (let id in state.dUsers) s.innerHTML += '<option value="' + state.dUsers[id].username + '">' + state.dUsers[id].username + '</option>'; 
    s.value = cv; 
}
...
export function searchStats() { 
    ...
    for (let id in state.dD) { 
        const o = state.dD[id]; 
        if (!o) continue; 
        if (user && o.settled_by !== user) continue; 
        if (!inRange(o.settled_at, from, to)) continue; 
    ...
    state.lastStats = { 
        targetUser: user || 'الكل', 
        dateFrom: from || 'البداية', 
        dateTo: to || 'اليوم', 
        invoices: invList, 
        summary: itemsSum, 
        totalSum: grand, 
        ordersCount: invList.length 
    }; 
    ...
    let h = '<h4 style="color:var(--accent);margin-bottom:12px;font-size:13px;"><i class="fas fa-receipt"></i> الفواتير (' + invList.length + '):</h4>' +
            '<div style="max-height:160px;overflow-y:auto;background:var(--bg-input);border-radius:10px;padding:10px;margin-bottom:18px;border:1px solid var(--border);">'; 
    invList.forEach(inv => { 
        const t = new Date(inv.settledAt).toLocaleTimeString('ar-IQ', { hour: '2-digit', minute: '2-digit' }), 
              d = new Date(inv.settledAt).toLocaleDateString('ar-IQ'); 
        h += '<div class="s-inv-row">' +
                '<span><b>' + (inv.tableNumber === 'سفري' ? 'تسوق مباشر' : 'طاولة ' + inv.tableNumber) + '</b> <small style="color:var(--text-muted);margin-right:6px;">(' + d + ' ' + t + ')</small></span>' +
                '<span style="color:var(--green);font-weight:700;">' + fmt(inv.totalPrice) + ' د.ع</span>' +
             '</div>'; 
    }); 
    h += '</div><h4 style="color:var(--teal);margin-bottom:12px;font-size:13px;"><i class="fas fa-box-open"></i> ملخص المواد:</h4>'; 
    Object.entries(itemsSum).forEach(([name, data]) => { 
        h += '<div class="s-item-card">' +
                '<div>' +
                    '<div class="s-item-name"><i class="fas fa-utensils" style="color:var(--teal);margin-left:5px;font-size:12px;"></i>' + name + '</div>' +
                    '<div class="s-item-count">العدد: ' + data.quantity + '</div>' +
                '</div>' +
                '<span class="s-item-total">' + fmt(data.total) + ' د.ع</span>' +
             '</div>'; 
    }); 
    h += '<div class="s-grand">' +
            '<div class="s-label">المجموع النهائي</div>' +
            '<div class="s-value">' + fmt(grand) + ' دينار عراقي</div>' +
         '</div>'; 
    c.innerHTML = h; 
}
...
export async function printStatsReport() { 
    ...
    const printers = window.App.getButtonPrinters('stats');
    ...
    const cv = document.createElement('canvas'), 
          cx = cv.getContext('2d'), 
          w = 576; 
    const ni = Object.keys(state.lastStats.summary).length, 
          nv = state.lastStats.invoices.length; 
    const ch = 280 + (nv * 28) + 50 + (ni * 32) + 160; 
    cv.width = w; 
    cv.height = Math.max(ch, 280); 
    cx.fillStyle = '#fff'; 
    cx.fillRect(0, 0, w, cv.height); 
    cx.fillStyle = '#000'; 
    cx.textBaseline = 'middle'; 
    cx.font = 'bold 26px "Segoe UI",Tahoma,sans-serif'; 
    cx.textAlign = 'center'; 
    cx.fillText(state.settings.restaurantName || 'المطعم', w / 2, 40); 
    cx.font = 'bold 20px "Segoe UI",Tahoma,sans-serif'; 
    cx.fillText('تقرير المبيعات', w / 2, 76); 
    cx.font = '14px "Segoe UI",Tahoma,sans-serif'; 
    cx.fillStyle = '#222'; 
    cx.fillText('المستخدم: ' + state.lastStats.targetUser + '  |  الفترة: ' + state.lastStats.dateFrom + ' - ' + state.lastStats.dateTo, w / 2, 110); 
    drawDash(cx, 20, w - 20, 130); 
    let y = 150; 
    cx.font = 'bold 16px "Segoe UI",Tahoma,sans-serif'; 
    cx.fillStyle = '#000'; 
    cx.textAlign = 'right'; 
    cx.fillText('الفواتير:', w - 30, y); 
    y += 24; 
    cx.font = '13px "Segoe UI",Tahoma,sans-serif'; 
    state.lastStats.invoices.forEach(inv => { 
        const t = new Date(inv.settledAt).toLocaleTimeString('ar-IQ', { hour: '2-digit', minute: '2-digit' }); 
        cx.textAlign = 'right'; 
        cx.fillText((inv.tableNumber === 'سفري' ? 'تسوق مباشر' : 'طاولة ' + inv.tableNumber) + ' (' + t + ')', w - 40, y); 
        cx.textAlign = 'left'; 
        cx.fillText(fmt(inv.totalPrice) + ' د.ع', 40, y); 
        y += 28; 
    }); 
    drawDash(cx, 25, w - 25, y); 
    y += 24; 
    cx.font = 'bold 16px "Segoe UI",Tahoma,sans-serif'; 
    cx.fillStyle = '#000'; 
    cx.textAlign = 'right'; 
    cx.fillText('ملخص المواد:', w - 30, y); 
    y += 24; 
    cx.font = 'bold 14px "Segoe UI",Tahoma,sans-serif'; 
    Object.entries(state.lastStats.summary).forEach(([name, data]) => { 
        cx.textAlign = 'right'; 
        cx.fillText(name + ' (' + data.quantity + ')', w - 40, y); 
        cx.textAlign = 'left'; 
        cx.fillText(fmt(data.total) + ' د.ع', 40, y); 
        y += 32; 
    }); 
    drawDash(cx, 20, w - 20, y); 
    y += 34; 
    cx.font = 'bold 20px "Segoe UI",Tahoma,sans-serif'; 
    cx.textAlign = 'right'; 
    cx.fillText('المجموع النهائي:', w - 30, y); 
    cx.textAlign = 'left'; 
    cx.fillText(fmt(state.lastStats.totalSum) + ' دينار', 30, y); 
    y += 40; 
    cx.font = 'italic 14px "Segoe UI",Tahoma,sans-serif'; 
    cx.textAlign = 'center'; 
    cx.fillText('نهاية التقرير', w / 2, y);

    const imgData = cv.toDataURL('image/jpeg', 0.85);
    let successCount = 0;
    let failCount = 0;
    
    toast(`جاري طباعة التقرير على ${printers.length} طابعة...`, 'ti');
    ...
    if (successCount > 0) {
        toast(`تمت طباعة التقرير على ${successCount} طابعة${failCount > 0 ? `، فشل ${failCount}` : ''}`, 'ts');
    } else {
        toast('فشلت طباعة التقرير', 'te');
    }
}

export function clearStats() { 
    if (document.getElementById('stUser')) document.getElementById('stUser').value = ''; 
    if (document.getElementById('stFrom')) document.getElementById('stFrom').value = ''; 
    if (document.getElementById('stTo')) document.getElementById('stTo').value = ''; 
...
export function sendCaptain() { 
    const i = document.getElementById('captainInput'), 
          t = i ? i.value.trim() : ''; 
    if (!t || !state.myUser) return; 
    playClickSnd(); 
    DB.pushData('captain_chats/' + state.myUser, { 
        text: t, 
        sender: 'cashier', 
        sender_name: state.myUser, 
