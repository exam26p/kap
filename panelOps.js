...
import { fmt, sameDay, playClickSnd, playOnce, alarmOrder, alarmMsg, toast, doBrowserPrint, drawDash } from './utils.js';
...
function markRead(tn) { const k = 'table_' + tn, ct = state.dMsgA[k] ? Object.keys(state.dMsgA[k]).length : 0; if (ct !== (state.dRead[k] || 0)) DB.setData('read_counters/' + k, ct); }

// تم إضافة export هنا لحل المشكلة
export function renderDetail() {
    ...
    bB.innerHTML = ''; if (oB) { for (let id in oB) { const s = oB[id].price * oB[id].quantity; total += s; bB.innerHTML += '<div class="inv-row"><span>' + oB[id].name + ' (x' + oB[id].quantity + ')</span><span style="color:var(--red);font-weight:700;">' + fmt(s) + ' د.ع</span></div>'; } } else bB.innerHTML = '<p>لا توجد طلبات جديدة.</p>';
    const bC = document.getElementById('dpBoxC'), oC = state.dC[k] && state.dC[k].processing_orders ? state.dC[k].processing_orders : null;
    bC.innerHTML = ''; if (oC) { for (let id in oC) { const s = oC[id].price * oC[id].quantity; total += s; bC.innerHTML += '<div class="inv-row"><span>' + oC[id].name + ' (x' + oC[id].quantity + ')</span><span style="color:var(--blue);">' + fmt(s) + ' د.ع</span></div>'; } } else bC.innerHTML = '<p>لا توجد وجبات في المطبخ.</p>';
...
function genReceipt(cfg, printerIp) {
    playOnce('print'); const cv = buildReceiptCanvas(cfg);
    if (printerIp && printerIp.trim()) { 
        const imgData = cv.toDataURL('image/jpeg', 0.7); // تقليل الجودة قليلاً لسيرفر الطباعة المحلي لكي لا يعطي خطأ 500
        fetch('http://localhost:5000/print-receipt', { 
            method: 'POST', 
            headers: { 'Content-Type': 'application/json' }, 
            body: JSON.stringify({ ip: printerIp, image: imgData }) 
        }).then(r => { 
            if (r.ok) { 
                toast('تم الإرسال للطابعة', 'ts'); 
            } else { 
                toast('فشل الإرسال للسيرفر - جاري الطباعة عبر المتصفح', 'te'); 
                doBrowserPrint(cv); 
            } 
        }).catch(() => { 
            toast('الطابعة غير متصلة - جاري الطباعة عبر المتصفح', 'te'); 
            doBrowserPrint(cv); 
        }); 
    } else { 
        doBrowserPrint(cv); 
...
export function printToKitchen() {
    const k = 'table_' + state.activeTable, oB = state.dB[k] && state.dB[k].sent_orders ? state.dB[k].sent_orders : null;
    if (!oB || !Object.keys(oB).length) { toast('لا توجد طلبات واردة', 'te'); return; }
    let items = []; for (let id in oB) items.push({ name: oB[id].name, qty: oB[id].quantity, price: oB[id].price });
    genReceipt({ title: 'بون التجهيز', tableNum: state.activeTable, dt: new Date().toLocaleString('ar-IQ', { hour12: true }), items, final: false, cashier: state.myUser, welcome: 'برجاء تجهيز الطلبات بأسرع وقت' }, state.routing.kitchenPrinterIp);
    for (let id in oB) DB.pushData('tablesC/' + k + '/processing_orders', { name: oB[id].name, price: oB[id].price, quantity: oB[id].quantity });
    DB.removeData('tablesB/' + k); toast('تم إرسال الطلبات للمطبخ', 'ts');
}

export function settleInvoice() {
    const k = 'table_' + state.activeTable, oB = state.dB[k] && state.dB[k].sent_orders ? state.dB[k].sent_orders : {}, oC = state.dC[k] && state.dC[k].processing_orders ? state.dC[k].processing_orders : {};
    if (!Object.keys(oB).length && !Object.keys(oC).length) { toast('لا توجد طلبات', 'te'); return; }
    let items = [], total = 0; const combined = { ...oB, ...oC }; for (let id in combined) { let s = combined[id].price * combined[id].quantity; total += s; items.push({ name: combined[id].name, qty: combined[id].quantity, price: combined[id].price }); }
    genReceipt({ title: 'إيصال الدفع', tableNum: state.activeTable, dt: new Date().toLocaleString('ar-IQ', { hour12: true }), items, final: true, total, cashier: state.myUser, welcome: 'نتمنى لكم تجربة مميزة، شكراً لاختياركم' }, state.routing.cashierPrinterIp);
    ...
    Promise.all([DB.removeData('tablesB/' + k), DB.removeData('tablesC/' + k), DB.removeData('msgA/' + k), DB.removeData('msgB/' + k), DB.removeData('read_counters/' + k)]).then(() => { toast('تم تسديد فاتورة ' + state.activeTable, 'ts'); closeDetail(); });
}
